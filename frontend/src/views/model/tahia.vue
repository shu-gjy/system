...
export default {
  ...
  methods: {
    download() {
      window.location.href = 'http://127.0.0.1:8001/api_tahia/tahia/downloadModel/'
    },
    ...
    init() {
      this.$store.dispatch('tahia/tahiaInitTrain').then(response => {
        console.log('This is training_data:', this.tahia_training_data)
        this.showChartCredit()

        this.loading = true
        this.$store.dispatch('tahia/tahiaInitTest').then(response => {
          this.abnormalCardList = this.tahia_test_data['cardlist']
    ...
    create() {
      this.$confirm('此操作将会运行模型并返回结果, 是否继续?', '提示', {
        ...
        this.$store.dispatch('tahia/tahiaTrain', this.form).then(response => {
          console.log('This is backData', this.tahia_training_data)
    ...
    testShow() {
      this.$confirm('此操作将会进行模型测试并返回结果, 是否继续?', '提示', {
        ...
        this.loading = true
        this.$store.dispatch('tahia/tahiaTest').then(response => {
    ...
    showChartCredit() {
      ...
      var option = {
        ...
        xAxis: {
          ...
          data: this.tahia_training_data['epoch']
        },
        yAxis: {
          type: 'value',
          min: 10,
        ...
        series: [
          {
            ...
            data: this.tahia_training_data['precision']
          },
          {
            name: 'Rec',
            type: 'line',
            ...
            data: this.tahia_training_data['recall']
          },
          {
            name: 'F1',
            type: 'line',
            ...
            data: this.tahia_training_data['f1']
          }
        ]
      }
          ...
          elements: [
            {
              ...
              style: {
                // text: '0.1194',
                text: this.tahia_test_data['precision'],
                fontSize: 20,
                fontWeight: 'bold',
                // lineDash: [0, 200],
                // lineDashOffset: 0,
          ...
          elements: [
            {
              ...
              style: {
                // text: '0.8888',
                text: this.tahia_test_data['recall'],
                fontSize: 20,
                fontWeight: 'bold',
                // lineDash: [0, 200],
                // lineDashOffset: 0,
